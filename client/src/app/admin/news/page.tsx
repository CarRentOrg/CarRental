...
export default function AdminNewsPage() {
    ...
    async function handleDelete(post: NewsPost) {
        if (!confirm(`Are you sure you want to delete "${post.title_en}"?`)) return;

        try {
            await api.news.delete(post.id);
            setNews(prev => prev.filter(p => p.id !== post.id));
        } catch (error) {
    ...
    const columns: Column<NewsPost>[] = [
        {
            ...
            cell: (row) => (
                <div className="flex items-center space-x-4">
                    <div className="relative h-12 w-20 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                        {row.image_url && <Image src={row.image_url} alt={row.title_en} fill className="object-cover" />}
                    </div>
                    <div>
                        <p className="text-sm font-bold text-gray-900 line-clamp-1">{row.title_en}</p>
                        <p className="text-xs text-gray-400 mt-0.5 max-w-[200px] truncate">{row.content_en}</p>
                    </div>
                </div>
            )
        },
        ...
        {
            header: "Date",
            cell: (row) => <span className="text-xs text-gray-500 font-bold">{new Date(row.created_at).toLocaleDateString()}</span>
        }
    ];
    ...
    return (
        <div className="space-y-8 pb-12">
            <AdminPageHeader
                ...
                breadcrumbs={[{ label: 'Dashboard', href: '/admin' }, { label: 'News' }]}
                actions={
                    <Link
                        href="/admin/news/new"
